...
def save_stimulus_and_analysis(stereo, freqs, flipped_noise, flipped_band, freq, flip_channel):
...
# === HEADPHONE CHECK ===
text_stim.text = "HEADPHONE CHECK:\nYou will hear tones in only one ear.\nPress left if you hear it on the left, right for right.\n\nPress space to start."
text_stim.draw()
win.flip()
event.waitKeys(keyList=['space'])

headphone_passed = True
for i in range(3):
    side = random.choice(['left', 'right'])
    tone = np.zeros((n_samples, 2))
    pure = np.sin(2 * np.pi * 1000 * np.linspace(0, duration, n_samples))
    if side == 'left':
        tone[:, 0] = pure
    else:
        tone[:, 1] = pure
    stim = sound.Sound(value=tone, sampleRate=sample_rate, stereo=True)
    stim.play()
    core.wait(duration)
    ...
    if keys[0] != side:
        headphone_passed = False

if not headphone_passed:
    text_stim.text = "You did not pass the headphone check.\nPlease try again with proper headphones."
    text_stim.draw()
    win.flip()
    core.wait(5.0)
    core.quit()
...
# === PRACTICE BLOCK ===
text_stim.text = "PRACTICE:\nYou will now hear exaggerated pitch illusions.\nPress space to continue."
text_stim.draw()
win.flip()
event.waitKeys(keyList=['space'])

practice_freqs = [400, 800]
for freq in practice_freqs:
    stim_array, freqs, flipped, band = create_dichotic_pitch(freq, wide=True)
    ...
    text_stim.draw()
    win.flip()
    resp = event.waitKeys(keyList=['y', 'n'])[0]

# === EXPERIMENT BLOCKS ===
for block_num in range(1, num_blocks + 1):
    rand_freq = random.choice(frequencies)
    test_freqs = random.choices([f for f in frequencies if f != control_freq], k=3)
    block_trials = [control_freq] + test_freqs + [control_freq]
    ...
    for trial_num, freq in enumerate(block_trials):
        ...
        if trial_type == 'Test':
            save_stimulus_and_analysis(stim_array, freqs, flipped_noise, flipped_band, freq, flip_channel)
        ...
        ear_correct = 'yes' if flip_channel == ear_resp else 'no' if flip_channel in ['left', 'right'] and ear_resp != 'NA' else 'NA'

        data_file.write(f"{block_num},{trial_num+1},{trial_type},{freq},{pitch_resp},{conf_resp},{rt:.3f},{ear_resp},{'high' if pitch_type_resp=='h' else 'low' if pitch_type_resp=='l' else 'NA'},{correct},{flip_channel},{ear_correct}\n")
        data_file.flush()
        core.wait(0.5)
