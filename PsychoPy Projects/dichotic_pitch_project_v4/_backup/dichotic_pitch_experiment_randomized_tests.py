...
for block_num in range(1, num_blocks + 1):
    ...
    for trial_num, freq in enumerate(block_trials):
        ...
        ear_correct = 'yes' if flip_channel == ear_resp else 'no' if flip_channel in ['left', 'right'] else 'NA'

        data_file.write(f"{block_num},{trial_num+1},{trial_type},{freq},{pitch_resp},{conf_resp},{rt:.3f},{ear_resp},{'high' if pitch_type_resp=='h' else 'low'},{correct},{flip_channel},{ear_correct}\n")
        data_file.flush()
        core.wait(0.5)
