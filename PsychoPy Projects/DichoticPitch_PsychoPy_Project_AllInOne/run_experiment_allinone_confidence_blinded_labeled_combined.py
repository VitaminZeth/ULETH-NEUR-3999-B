...
def ask_with_confidence(main_msg, main_keys, conf_labels=CONF_LABELS):
    """Show ONE screen, collect main response, then confidence 1-5.

    Returns: (main_key, main_rt, conf_key, conf_rt)
      - main_rt: seconds from prompt onset to main key
      - conf_rt: seconds from *after main key* to confidence key
    """
    labels_line = " | ".join([f"{k}={v}" for k, v in conf_labels.items()])
    combined = (
        f"{main_msg}\n\n"
        f"Then rate confidence (press 1-5):\n{labels_line}"
    )
    ...
    # main response RT from prompt onset
    ...
    # confidence RT from AFTER main response
