...
# --- Step X: Apply low-pass filter with 1200 Hz cutoff and 50 ms half-Gaussian ramp ---
...
def apply_half_gaussian_ramp(signal, fs, ramp_duration_ms=50):
    ramp_samples = int((ramp_duration_ms / 1000.0) * fs)
    ramp = windows.gaussian(2 * ramp_samples, std=ramp_samples / 3)
    half_ramp = ramp[:ramp_samples]
    ramp_up = half_ramp
    ramp_down = half_ramp[::-1]
...
bandpassed = apply_half_gaussian_ramp(lowpass_filter(bandpassed, 1200, fs), fs)
notched = apply_half_gaussian_ramp(lowpass_filter(notched, 1200, fs), fs)
bandpassed2 = apply_half_gaussian_ramp(lowpass_filter(bandpassed2, 1200, fs), fs)
notched2 = apply_half_gaussian_ramp(lowpass_filter(notched2, 1200, fs), fs)

# Re-normalize after filtering and ramping
bandpassed /= np.max(np.abs(bandpassed))
notched /= np.max(np.abs(notched))
bandpassed2 /= np.max(np.abs(bandpassed2))
notched2 /= np.max(np.abs(notched2))
