...
txt.text = "Adjust volume comfortably\n▲/▼ to change, SPACE to confirm"
txt.draw(); win.flip()

# allow up/down adjustment here too
while True:
    txt.text = f"Volume: {int(volume*100)}%\n▲ Increase  ▼ Decrease\nSPACE = OK\nS = Skip tests\nESC = Quit"
    txt.draw(); win.flip()
...
if not skip_hp:
    # generate continuous 1 kHz tone
    ...
    if not passed:
        ...
        # you could wrap in a while loop to retry; omitted for brevity
else:
...
pure_freqs = [20, 250, 400, 600, 800, 1000, 2000, 4000, 8000, 16000, 20000]
    ...
    for ear in ['Left','Right']:
        ...
        for f in pure_freqs:
            ...
            while True:
                ...
                sound.Sound(value=tone, sampleRate=SAMPLE_RATE, stereo=True).setVolume(volume).play()
                core.wait(DURATION)
                ...
                if k=='y':
                    heard_once=True
                    if lvl<=0:
                        # play pink noise boost test
                        # pink noise generation & play omitted for brevity...
                        # log false positive if they still respond
                        pass
                    lvl -= 10
                else:
                    if heard_once:
...
# record zero-block screening row
...
def run_phase(compensated):
    ...
    for blk in range(1,NUM_BLOCKS+1):
        ...
        for tr,f in enumerate(seq,1):
            ...
            if key=='y':
                txt.text="Which ear? (L/R)"; txt.draw(); win.flip()
